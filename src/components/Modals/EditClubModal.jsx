...
import { Shield, X, Save, Upload } from 'lucide-react';
...
export const EditClubModal = ({ isOpen, onClose, club, onSave }) => {
  ...
  useEffect(() => {
    if (club) {
      ...
      setStadium(club.stadium || 'Estadio Municipal');
      setLogo(club.logo || '⚽');
      setColor(club.color || '#10B981');
    }
  }, [club]);
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950/50">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-bold text-lg text-white">Editar Información del Club</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 uppercase mb-1">Nombre del Entrenador / Mánager</label>
            <input
              type="text"
              required
              value={managerName}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 uppercase mb-1">Nombre del Estadio / Campo 🏟️</label>
            <input
              type="text"
              required
              placeholder="Ej. Estadio Municipal de Butarque"
              value={stadium}
